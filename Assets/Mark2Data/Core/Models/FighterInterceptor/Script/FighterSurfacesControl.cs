...
namespace AirPlane.Controler 
{
    public class FighterSurfacesControl : MonoBehaviour
    {
        [HideInInspector] public float VerticalVal = 0;
        [HideInInspector] public float HorizontalVal = 0;
        ...
        void FixedUpdate()
        {
            MRx = Mathf.Clamp(MRx + VerticalVal * 0.05f, -1f, 1f);
            MRz = Mathf.Clamp(MRz - HorizontalVal * 0.05f, -1f, 1f);
            MRy = Mathf.Clamp(MRy + YawVal * 0.05f, -1f, 1f);
            ...
            //    }
            //}
            rightElevator.transform.localRotation = Quaternion.Euler(new UnityEngine.Vector3((MRz + MRx) * maxAngle, rightElevatorSetAngle, 0));
            leftElevator.transform.localRotation = Quaternion.Euler(new UnityEngine.Vector3((-MRz + MRx) * maxAngle, leftElevatorSetAngle, 0));
