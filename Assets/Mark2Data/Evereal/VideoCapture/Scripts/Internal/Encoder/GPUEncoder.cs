    ...
    IEnumerator BlitCubemapTextures()
    {
      ...
      if (captureStarted || screenshotStarted)
      {
        ...
        if (projectionType == ProjectionType.EQUIRECT)
        {
          ...
          {
                        equirectMaterial.SetMatrix("_CubeTransform", Matrix4x4.TRS(UnityEngine.Vector3.zero, transform.rotation, UnityEngine.Vector3.one));
          }
          else
          {
        ...
        else if (projectionType == ProjectionType.CUBEMAP)
        {
          ...
          if (includeCameraRotation)
          {
                        ...
                        cubemapMaterial.SetMatrix("_CubeTransform", Matrix4x4.TRS(UnityEngine.Vector3.zero, transform.rotation, UnityEngine.Vector3.one));
          }
          else
          {
