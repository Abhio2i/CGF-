...
// Allowing the manuevers can be performed 
...
namespace Vazgriz.Plane
{
    public class Plane : MonoBehaviour
    {
        ...
        //Allow To perform Maneuver
        bool ManeuverFun()
        {
            ManeuverControls maneuver_controls = GetComponent<ManeuverControls>();
            DicisonTree maneuverConditions = GetComponent<DicisonTree>();
            //maneuverConditions.enemy = GetComponent<AIController>().targetPos;
           
            //if plane is not doing any maneuver
            if (maneuver_controls.initial)
            {
                Maneuvers maneuvers = maneuverConditions.Ask(GetComponent<AIController>().targetPos);
                //maneuvers = maneuver_controls.maneuver;
                // if dicison tree give any maneuver without null
                if (maneuvers != Maneuvers.none)
                {
                    ...
                    if (maneuvers == Maneuvers.highYoyo || maneuvers == Maneuvers.lowYoyo)
                    {
                        maneuver_controls.direction = maneuverConditions.direction;
                        GetComponent<AIController>().offset = new Vector3(0, 0, 50f);//leadLagPursuit
                    }
                    else
                    {
                        GetComponent<AIController>().offset = new Vector3(0, 0, 0f);
                    }
                    maneuver_controls.maneuver = maneuvers;
                    maneuver_controls.Perform();
                    return true;
                }
                else
                {
                    Rigidbody.isKinematic = false;
                    //if decision tree not give maneuver == none
                    Rigidbody.constraints = RigidbodyConstraints.None;
                    Rigidbody.freezeRotation = false;
                    maneuver_controls.maneuver = maneuvers;
                    return false;
