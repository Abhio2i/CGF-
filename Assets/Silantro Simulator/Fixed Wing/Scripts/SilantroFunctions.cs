    ...
    public class FMath
    {
        ...
        public static void PlotAirfoil(Vector3 leadingPoint, Vector3 trailingPoint, SilantroAirfoil foil, Transform foilTransform, out float foilArea, out List<Vector3> points)
        {
            ...
            if (foil.x.Count > 0)
            {
                for (int j = 0; (j < foil.x.Count); j++)
                {
                    ...
                    Vector3 liftDirection = foilTransform.up.normalized;
                    PointA = XA + (liftDirection * ((foil.y[j]) * chordDistance));
                    points.Add(PointA);
                    if ((j + 1) < foil.x.Count) { Vector3 XB = (leadingPoint - ((leadingPoint - trailingPoint).normalized * (foil.x[j + 1] * chordDistance)));
        ...
        public static void PlotRibAirfoil(Vector3 leadingPoint, Vector3 trailingPoint, float distance, float wingTip, Color ribColor, bool drawSplits, SilantroAirfoil rootAirfoil, SilantroAirfoil tipAirfoil, float span, Transform foilTransform)
        {
            ...
            if (rootAirfoil.x.Count > 0)
            {
                ...
                {
                    float xi = MathBase.EstimateEffectiveValue(rootAirfoil.x[j], tipAirfoil.x[j], distance, wingTip, span);
                    float yi = MathBase.EstimateEffectiveValue(rootAirfoil.y[j], tipAirfoil.y[j], distance, wingTip, span);
                    //BASE POINT
                    Vector3 XA = leadingPoint - ((leadingPoint - trailingPoint).normalized * (xi * chordDistance));
                    Vector3 liftDirection = foilTransform.up; PointXA = XA + (liftDirection * yi * chordDistance); points.Add(PointXA);
