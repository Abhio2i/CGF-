...
public class SilantroCamera : MonoBehaviour
{
    ...
    public float orbitAngle = 180.0f;
    ...
    public float m_orbitSpeed = 1;
    public float m_verticalSpeed = 0.65f;
    //------------------------------------------------------------------------------------------
    void OrbitCamera()
    {
        ...
        if (Input.GetMouseButton(0) && Application.isFocused)
        {
            if (FirstClick)
            {
                basePositionition = Input.mousePosition;
                FirstClick = false;
            }
            if (Application.isFocused)
            {
                orbitAngle += (Input.mousePosition - basePositionition).x * Time.deltaTime * m_orbitSpeed;
                if(orbitAngle > 180) { orbitAngle = -180; }
                if(orbitAngle < -180) { orbitAngle = 180; }
            }
        }
        ...
        //-----------------PROCESS VARIABLES
        cameraRange = focusPoint.forward;
        cameraRange.y = 0f;
        cameraRange.Normalize();
        ...
        if (cameraFocus == CameraFocus.Normal) { 
            ...
            normalExterior.transform.LookAt(focusPoint.position); }
