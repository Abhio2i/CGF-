...
public class SilantroFlightComputer : MonoBehaviour
{
    ...
    public enum TurnControl { Basic, Dual}
    public TurnControl turnControl = TurnControl.Basic;
    ...
    public SilantroPID turnClimbSolver;
    public SilantroPID turnAltitudeSolver;
    ...
    public void InitializeComputer()
    {
        ...
        if (allOk)
        {
            ...
            sP = 101.5f * 1000f * 0.020885434273f;
    ...
    void GeneralControl()
    {
        ...
        if (autoSlat == ControlState.Active)
        {
            ...
            // -------------------------------------- Alpha based actuation
            float T = (controller.core.ambientTemperature + 273.15f) * 1.8f;
            float ρ = controller.core.airDensity * 0.00194032033f;
            float ν = controller.core.airDensity * 3.28084f;
            dP = 0.5f * ρ * Mathf.Pow(ν, 2);
            float δQ = dP / sP;

            α_e = controller.core.α;
            if (float.IsNaN(α_e) || float.IsInfinity(α_e)) { α_e = 0f; }
            β_e = controller.core.β;
            if (float.IsNaN(β_e) || float.IsInfinity(β_e)) { β_e = 0f; }
            le_rate = (α_e - le_dot) * 7.25f;
            le_integral += (le_rate * Time.fixedDeltaTime);
            if (float.IsNaN(le_integral) || float.IsInfinity(le_integral)) { le_integral = 0f; }
            le_dot = le_integral + maximumWingAlpha * 2.0f;
            if (float.IsNaN(le_dot) || float.IsInfinity(le_dot)) { le_dot = 0f; }
            δLE = (le_dot * 1.38f) + 1.45f - (9.05f * δQ);
            if (δLE > performanceSlat) { δLE = performanceSlat; }
            if (δLE < 0) { δLE = 0f; }
            if (knotSpeed > 40f)
            {
                foreach (SilantroAerofoil foil in wingFoils)
                {
                    if (foil.slatState == SilantroAerofoil.ControlState.Active) { foil.baseSlat = Mathf.MoveTowards(foil.baseSlat, δLE, foil.slatActuationSpeed * Time.fixedDeltaTime); }
                }
            }
        }
    ...
    public float sP;
    public float dP;
    public float le_rate;
    public float le_integral;
    public float le_dot;
    public float δLE;
    ...
    private void AssitedControl()
    {
        ...
        if (controlType == ControlType.ThreeAxis)
        {
            ...
            else
            {
                ...
                if (headingHold == ControlState.Active && Mathf.Abs(headingError) > 5f)
                {
                    turnAltitudeSolver.maximum = presetClimbLimit; turnAltitudeSolver.minimum = -presetClimbLimit;
                    commandClimbRate = turnAltitudeSolver.CalculateOutput(altitudeError, timeStep) * MathBase.toFtMin;
                }
                else
                {
                ...
                if (headingHold == ControlState.Active && Mathf.Abs(headingError) > 5f && turnControl == TurnControl.Dual)
                {
                    turnClimbSolver.minimum = -balancePitchRate; turnClimbSolver.maximum = balancePitchRate;
                    commandPitchRate = turnClimbSolver.CalculateOutput(climbRateError, timeStep);
                }
                else
                {
