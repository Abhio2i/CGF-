...
public class AerofoilDesign
{
    ...
    public static void ShapeAerofoilPlain(SilantroAerofoil foil)
    {
        ...
        {
            // ----------------- Division Factors
            float currentSection = (float)p, nextSection = (float)(p + 1);
            float sectionLength = (float)foil.foilSubdivisions;
            float sectionFactor = currentSection / sectionLength;
            Vector3 LeadingPointA, TrailingPointA;

            // ---------------- Points
            TrailingPointA = MathBase.EstimateSectionPosition(foil.RootChordTrailing, foil.TipChordTrailing, sectionFactor); 
            LeadingPointA = MathBase.EstimateSectionPosition(foil.RootChordLeading, foil.TipChordLeading, sectionFactor);
    ...
    public static void EstimateControlSurface(SilantroAerofoil foil, float inputTipChord, float inputRootChord, bool[] sections, Color surfaceColor, bool leading, bool floating, float surfaceDeflection, bool drawSections, out float surfaceSpan, out float surfaceArea)
    {
        ...
        if (sections != null)
        {
            for (int i = 0; i < sections.Length; i++)
            {
                ...
                {
                    //BUILD VARIABLES
                    float currentSelection = (float)i; 
                    float nextSection = (float)(i + 1);
                    float sectionLength = (float)sections.Length;
                    float baseFactorA = currentSelection / sectionLength;
                    float baseFactorB = nextSection / sectionLength;
                    ...
                    else
                    {
                        if (!leading)
                        {
                            ...
                            rects[0] = (MathBase.EstimateSectionPosition(
                                MathBase.EstimateSectionPosition(foil.RootChordTrailing, foil.RootChordLeading, (inputRootChord * 0.01f)),
                                MathBase.EstimateSectionPosition(foil.TipChordTrailing, foil.TipChordLeading, (inputTipChord * 0.01f)), baseFactorA));

                            rects[3] = (MathBase.EstimateSectionPosition(
                                MathBase.EstimateSectionPosition(foil.RootChordTrailing, foil.RootChordLeading, (inputRootChord * 0.01f)),
                    ...
                    //DEFLECT SURFACE
                    rects[1] = rects[0] + Quaternion.AngleAxis(surfaceDeflection, (rects[3] - rects[0]).normalized) * (rects[1] - rects[0]); //m_rootTrailing
                    rects[2] = rects[3] + (Quaternion.AngleAxis(surfaceDeflection, (rects[3] - rects[0]).normalized)) * (rects[2] - rects[3]);//m_tipTrailing
