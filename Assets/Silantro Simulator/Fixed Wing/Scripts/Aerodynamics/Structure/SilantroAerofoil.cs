...
public class SilantroAerofoil : MonoBehaviour
{
    ...
    [Range(5, 50f)] public float trimAuthorityPositive = 30f;
    [Range(5, 50f)] public float trimAuthorityNegative = 30f;
    ...
    public float positiveTrimLimit = 5f, negativeTrimLimit = 3f, trimInput, trimDeflection, trimTabDeflection;
    public float flapA1 = 10f, flapA2 = 30f, maximumSpoilerDeflection = 60f;
    public float positiveTabLimit = 10f, negativeTabLimit = 10f;
    ...
    public RotationAxis trimDeflectionAxis = RotationAxis.X;
    ...
    public DeflectionDirection trimDeflectionDirection = DeflectionDirection.CW;
    ...
    public Vector3 trimAxis; Quaternion baseTrimRotation;
    ...
    public void InitializeFoil()
    {
        ...
        if (allOk)
        {
            ...
            AnalyseDimensions(); 
            if (aerofoilType != AerofoilType.Balance && controlState == ControlType.Controllable) { AerofoilDesign.PlotControlCurves(foil, aerofoilType == AerofoilType.Wing && slatState == ControlState.Active); }
            ...
            if (controlState == ControlType.Controllable)
            {
                ...
                if (trimState == TrimState.Available && trimTabModel != null) { baseTrimRotation = trimTabModel.transform.localRotation; }
                ...
                trimAxis = Handler.EstimateModelProperties(trimDeflectionDirection.ToString(), trimDeflectionAxis.ToString());
    ...
    private void AnalyseDimensions()
    {
        ...
        if (deflectionType == DeflectionType.Asymmetric) { positiveTrimLimit = (trimAuthorityPositive / 100f) * positiveLimit; negativeTrimLimit = (trimAuthorityNegative / 100f) * negativeLimit; }
        else { positiveTrimLimit = (trimAuthorityPositive / 100f) * positiveLimit; negativeTrimLimit = (trimAuthorityNegative / 100f) * positiveLimit; }
    ...
    void AnalyseStructure()
    {
        ...
        //Gizmos.DrawSphere(tipChordCenter, 0.1f);
    ...
    void AnalyseControl()
    {
        if (controlState == ControlType.Controllable && surfaceType != SurfaceType.Inactive)
        {
            ...
            if (deflectionType == DeflectionType.Symmetric)
            {
                ...
                {
                    trimDeflection = trimInput > 0f ? trimInput * positiveTrimLimit : trimInput * positiveTrimLimit;
                    trimTabDeflection = trimInput > 0f ? trimInput * positiveTabLimit : trimInput * positiveTabLimit;
                }
                else { trimDeflection = 0f; trimTabDeflection = 0f; }
            ...
            if (deflectionType == DeflectionType.Asymmetric)
            {
                if (transform.localScale.x < 0)
                {
                    ...
                    if (trimState == TrimState.Available)
                    {
                        trimDeflection = trimInput > 0f ? trimInput * negativeTrimLimit : trimInput * positiveTrimLimit;
                        trimTabDeflection = trimInput > 0f ? trimInput * negativeTabLimit : trimInput * positiveTabLimit;
                    }
                    else { trimDeflection = 0f; trimTabDeflection = 0f; }
                    controlActuatorDeflection = baseDeflection + trimDeflection;
                ...
                if (transform.localScale.x > 0)
                {
                    ...
                    if (trimState == TrimState.Available)
                    {
                        trimDeflection = trimInput > 0f ? trimInput * positiveTrimLimit : trimInput * negativeTrimLimit;
                        trimTabDeflection = trimInput > 0f ? trimInput * positiveTabLimit : trimInput * negativeTabLimit;
                    }
                    else { trimDeflection = 0f; trimTabDeflection = 0f; }
                    controlActuatorDeflection = baseDeflection + trimDeflection;
                    if (controlActuatorDeflection > positiveLimit) { controlActuatorDeflection = positiveLimit; }
            ...
            else
            {
                ...
                else
                {
                    if (airbrakeActive && connectedAircraft.flightComputer.airbrakeType != SilantroFlightComputer.AirbrakeType.ActuatorOnly)
                    {
                        if (surfaceType == SurfaceType.Ruddervator)
                        {
                            if (transform.localScale.x > 0) { controlDeflection = -connectedAircraft.flightComputer.currentAileronSet; }
            ...
            if (trimTabModel) { trimTabModel.transform.localRotation = baseTrimRotation; trimTabModel.transform.Rotate(trimAxis, trimTabDeflection); }
    ...
    protected void AnalyseForces()
    {
        ...
        for (int p = 0; p < foilSubdivisions; p++)
        {
            ...
            LeadCenter = MathBase.EstimateSectionPosition(LeadEdgeLeft, LeadEdgeRight, 0.5f);
            TrailCenter = MathBase.EstimateSectionPosition(TrailEdgeLeft, TrailEdgeRight, 0.5f);
            ...
            // ---------------------------------- AOA
            Vector3 panelWorldFlow = connectedAircraft.aircraft.GetPointVelocity(geometricCenter);
            airflow = -panelWorldFlow;
            ...
            airflow += circulation;
            parallelFlow = (transform.right * (Vector3.Dot(transform.right, airflow)));
            airflow -= parallelFlow;
            ...
            float panelLift = sweepCorrectionFactor * spoilerFactor * panelArea * panelLiftCoefficient * dynamicPressure;
            TotalLift += panelLift;
            pxcl = panelLiftCoefficient;
            vortexLiftPercentage = vxcl / pxcl;
            if (liftMethod == LiftMethod.Corrected)
            {
                panelLiftForce = Vector3.Cross(transform.right, airflow);
                panelLiftForce.Normalize();
                panelLiftForce *= panelLift;
            }
            else
            {
                panelLiftForce = liftDirection * panelLift;
            }
            ...
            //Debug.DrawLine(geometricCenter, geometricCenter + (panelLiftForce * Time.deltaTime * 0.03f), Color.green);
            //Debug.DrawLine(geometricCenter, geometricCenter + (panelDragForce * Time.deltaTime * 0.03f), Color.red);
