...
public class SilantroGun : MonoBehaviour
{
	...
	public Position gunPosition = Position.External;
	...
	public SilantroActuator gunCover;
	...
	{ 
		if (allOk)
		{
			...
			if(gunPosition == Position.Internal && gunCover != null)
            {
				if (running && gunCover.actuatorState == SilantroActuator.ActuatorState.Disengaged) { gunCover.EngageActuator(); }
				if (!running && gunCover.actuatorState == SilantroActuator.ActuatorState.Engaged) { gunCover.DisengageActuator(); }
			}
	...
	public void Fire()
	{
		...
		if (muzzles.Length > 0 && !Override)
		{
			...
			if (controller !=null && controller.aircraft != null) { baseVelocity = controller.aircraft.velocity; }
...
public class SilantroGunEditor : Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		GUILayout.Space(3f);
		EditorGUILayout.PropertyField(serializedObject.FindProperty("gunPosition"), new GUIContent("Position"));
		if (gun.gunPosition == SilantroGun.Position.Internal)
		{
			GUILayout.Space(3f);
			EditorGUILayout.PropertyField(serializedObject.FindProperty("gunCover"), new GUIContent("Actuator"));
		}
