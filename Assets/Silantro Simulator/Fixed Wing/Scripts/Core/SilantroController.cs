...
public class SilantroController : MonoBehaviour
{
    ...
    /// </summary>
    [Tooltip("A float using the Range attribute")] public SoundState currentSoundState = SoundState.Exterior;
    ...
    /// </summary>
    public enum InputType { Default, Custom, VR, Mobile ,AI, UDP}
    ...
    void InitializeAircraft()
    {
        ...
        if (allOk)
        {
            ...
            // --------------------------- Bulbs
            foreach (SilantroLight bulb in lightBulbs)
            {
                bulb.InitializeBulb(); 
                if (bulb.lightType == SilantroLight.LightType.Landing && gearActuator != null)
                {
