...
public class SilantroWaypointCircuit : MonoBehaviour
{
    ...
    public enum SinglePathType { Taxi, Runway , Custom}
    public SinglePathType singlePathType = SinglePathType.Taxi;
    public enum CircuitPathType { Dubins, Spline }
    public CircuitPathType circuitPathType = CircuitPathType.Spline;
    ...
    public float baseHeading, topHeading;

    public float turnRadius = 1000f;
    public float stepDistance = 100f;
    ...
    private void CachePositionsAndDistances()
    {
        ...
        if(waypointType == WaypointType.SinglePath && singlePathType == SinglePathType.Runway)
        {
            if(waypointList.items.Count > 2)
            {
                baseHeading = waypointList.items[0].eulerAngles.y;
                topHeading = waypointList.items[waypointList.items.Count - 1].eulerAngles.y;
            }
        }
    }
    ...
    public void CreateCurve(List<Vector3> controlPoints)
    {
        ...
        for (int s = 0; s < controlPoints.Count - 3; s += 3)
        {
            ...
            for (int p = 0; p < (resolution / segments); p++)
            {
                ...
                Vector3 point = MathBase.BezierPathCalculation(p0, p1, p2, p3, t);
    ...
    private void DrawCircuit(bool selected)
    {
        ...
        float headHeight, endHeight;
        if (waypointType == WaypointType.SinglePath && singlePathType == SinglePathType.Runway) { Gizmos.color = Color.yellow; headHeight = 20f; endHeight = 28f; }
        else { Gizmos.color = Color.red; headHeight = 10f; endHeight = 15f; }

        Handles.color = Color.green; Handles.ArrowHandleCap(0, waypointList.items[0].position, transform.rotation * Quaternion.LookRotation(Vector3.up), headHeight, EventType.Repaint);
        ...
        Handles.color = Color.red; Handles.ArrowHandleCap(0, endPoint, transform.rotation * Quaternion.LookRotation(Vector3.up), endHeight, EventType.Repaint);

        // ----------------------------------------------- Draw Circuit
        if (waypointType == WaypointType.Circuit)
        {

            if (circuitPathType == CircuitPathType.Spline)
            {
            ...
            if(circuitPathType == CircuitPathType.Dubins)
            {
                pathPoints = new List<Vector3>();
                if (waypointList.items.Count > 1)
                {
                    for (int i = 0; i < waypointList.items.Count - 1; i++)
                    {
                        Transform pointA = waypointList.items[i];
                        Transform pointB = waypointList.items[i + 1];

                        //GeneratePath(pointA, pointB);
                    }
                   // GeneratePath(waypointList.items[waypointList.items.Count - 1], waypointList.items[0]);
                }
        ...
        if (waypointType == WaypointType.SinglePath && singlePathType == SinglePathType.Runway)
        {
            if (waypointList.items.Count > 2)
            {
                DrawGlideSlope(waypointList.items[0].position, 10000, 177f, Color.green);
                DrawGlideSlope(waypointList.items[0].position, 10000, 174f, Color.red);

                DrawGlideSlope(waypointList.items[waypointList.items.Count - 1].position, 10000, 3f, Color.green);
                DrawGlideSlope(waypointList.items[waypointList.items.Count - 1].position, 10000, 6f, Color.red);
            }
        }
#endif
    }
    ...
    // ----------------------------------------------------------------------------------------------------------------------------------------------------------
    void GeneratePath(Transform pointA, Transform pointB)
    {
        SilantroPath path = new SilantroPath();

        if (pointA != null && pointB != null)
        {
            Vector3 basePoint = pointA.position;
            Vector3 finalPoint = pointB.position;
            float baseHeading = pointA.eulerAngles.y * Mathf.Deg2Rad;
            float finalHeading = pointB.eulerAngles.y * Mathf.Deg2Rad;

            path = PathGenerator.GetPath(basePoint, baseHeading, finalPoint, finalHeading, turnRadius, stepDistance);
        }

        if(path != null && path.pathVectorPoints.Count > 1)
        {
            foreach(Vector3 point in path.pathVectorPoints) { pathPoints.Add(point); }
        }
    }


    void DrawGlideSlope(Vector3 point, float extension, float slope, Color glideColor)
    {
        var line = point + (transform.forward * extension);
        var rotatedLine = Quaternion.AngleAxis(-slope, transform.right) * line;
        Debug.DrawLine(point, rotatedLine, glideColor);
    }
}
...
public class SilantroWaypointCircuitEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if(circuit.waypointType == SilantroWaypointCircuit.WaypointType.SinglePath)
        {
            GUILayout.Space(3f);
            EditorGUILayout.PropertyField(serializedObject.FindProperty("singlePathType"), new GUIContent(" "));
        }
        if (circuit.waypointType == SilantroWaypointCircuit.WaypointType.Circuit)
        {
            GUILayout.Space(3f);
            EditorGUILayout.PropertyField(serializedObject.FindProperty("circuitPathType"), new GUIContent(" "));
        }
        GUILayout.Space(10f);
        ...
        if(circuit.waypointType == SilantroWaypointCircuit.WaypointType.SinglePath && circuit.singlePathType == SilantroWaypointCircuit.SinglePathType.Runway)
        {
            GUILayout.Space(3f);
            EditorGUILayout.LabelField("Top Heading", circuit.topHeading.ToString("0.0") + " °");
            GUILayout.Space(3f);
            EditorGUILayout.LabelField("Base Heading", circuit.baseHeading.ToString("0.0") + " °");
        }
