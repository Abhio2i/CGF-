...
public class SilantroInput
{
    ...
    {
        SilantroControl controlSource = new SilantroControl();
        ...
        controlSource.General.AfterburnerSwitch.performed += ctx =>
        {
            ...
            if (button.isPressed) { if (!boostRunning) { EngageBoost(); } else if (boostRunning) { DisEngageBoost(); } }
        };
    ...
    public void CollectInputs()
    {
        ...
        else if (controller.inputType == SilantroController.InputType.Mobile)
        {
            ...
            // rawYawInput = //Your mobile Yaw output
            // rawThrottleInput = //Your mobile throttle output
        }

        else if(controller.inputType == SilantroController.InputType.VR)
        {
            rawRollInput = controller.controlStick.rollOutput;
            rawPitchInput = controller.controlStick.pitchOutput;
            rawThrottleInput = controller.throttleLever.controlOutput;
        }

        else if(controller.inputType == SilantroController.InputType.Custom)
        {
            // rawPitchInput = //Your cube pitch output
            // rawRollInput = //Your cube roll output
            // rawYawInput = //Your cube Yaw output
            // rawThrottleInput = //Your cube throttle output
            if (Input.GetButtonDown("Start Engine")) { TurnOnEngines(); }
            if (Input.GetButtonDown("Stop Engine")) { TurnOffEngines(); }
            if (Input.GetButtonDown("Start Engine BandL")) { TurnOnLeftEngines(); }
            if (Input.GetButtonDown("Start Engine BandR")) { TurnOnRightEngines(); }
            ...
            if (controller.flightComputer != null && controller.flightComputer.operationMode != SilantroFlightComputer.AugmentationType.Autonomous)
            {
                ...
                if (Joystick.all.Count > 0)
                {
                    ...
                    if (temp.y < 0)
                        ...
                        temp.y = -1 * (1 - temp.y);
                    rawRollInput = temp.x < 0 ? -Mathf.Pow(temp.x, 2f) : Mathf.Pow(temp.x, 2f);
                    rawPitchInput = temp.y;
                    //_rawYawInput = 0f;
                }
    ...
    public void UpdateInput()
    {
        ...
        if (Application.isFocused || true)  //true added manually to controll
        {
