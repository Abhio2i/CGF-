...
public class SilantroCore : MonoBehaviour
{
    ...
    private void ProcessData()
    {
        ...
        // ---------------- PERFORMANCE
        Vector3 localVelocity = aircraft.transform.InverseTransformDirection(aircraft.velocity);
        ...
        α = Mathf.Atan(localVelocity.y / localVelocity.z) * Mathf.Rad2Deg * -1f;
        float v = Mathf.Sqrt(Mathf.Pow(localVelocity.x, 2) + Mathf.Pow(localVelocity.y, 2) + Mathf.Pow(localVelocity.z, 2));
        β = Mathf.Asin(localVelocity.x / v) * Mathf.Rad2Deg;
