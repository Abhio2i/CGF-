...
namespace Oyedoyin
{
    public class Handler
    {
        ...
        public static void ControlAxis(Axis axis, bool duplicate)
        {
            ...
            {
                if (axisIter.FindPropertyRelative("m_Name").stringValue == axis.name && !duplicate)
                {
    ...
    public static class MathBase
    {
        ...
        public static float EstimateSection(float lhs, float rhs, float factor) { float estimatedPosition = lhs + ((rhs - lhs) * factor); return estimatedPosition; }
        ...
        /// </returns>
        // ----------------------------------------------------------------------------------------------------------------------------------------------------------
        public static double EstimateRe(double inputSpeed, double chord, double airDensity, double viscocity, double machSpeed, double k)
        {
            double Re1 = (airDensity * inputSpeed * chord) / viscocity; double Re2;
            if (machSpeed < 0.9f) { Re2 = 38.21f * Math.Pow(((chord * 3.28f) / (k / 100000)), 1.053f); }
            else { Re2 = 44.62f * Math.Pow(((chord * 3.28f) / (k / 100000)), 1.053f) * Math.Pow(machSpeed, 1.16f); }
        ...
        // ----------------------------------------------------------------------------------------------------------------------------------------------------------
        public static float EstimateSkinDragCoefficient(double Cr, double Ct, double velocity, double airDensity, double viscocity, double machSpeed, double k)
        {
            double Recr = EstimateRe(velocity, Cr, airDensity, viscocity, machSpeed, k);
            double Rect = EstimateRe(velocity, Ct, airDensity, viscocity, machSpeed, k);
            double X0Cr1 = 36.9f * 0.366f * Math.Pow((1 / Recr), 0.375f);//Root Upper
            double X0Ct1 = 36.9f * 0.366f * Math.Pow((1 / Rect), 0.375f);//Tip Upper
            double X0Ct2 = 36.9f * 0.237f * Math.Pow((1 / Rect), 0.375f);//Tip Upper

            double Cf1 = (0.074f / Math.Pow(Recr, 0.2f)) * Math.Pow((1 - (0.2f - X0Cr1)), 0.8f);
            double Cfu2 = (0.074f / Math.Pow(Rect, 0.2f)) * Math.Pow((1 - (0.2f - X0Ct1)), 0.8f);
            double Cfl2 = (0.074f / Math.Pow(Rect, 0.2f)) * Math.Pow((1 - (0.1f - X0Ct2)), 0.8f);//X0Ct1 TO Reduce...Check Later
            double Cf2 = (Cfu2 + Cfl2) / 2f;
            return (float)(Cf1 + Cf2) / 2f;
        }
